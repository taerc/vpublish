...
CREATE TABLE IF NOT EXISTS `error_records` (
    ...
    `error_message` TEXT NOT NULL COMMENT '接口返回的错误消息信息',
    ...
    `remark` TEXT DEFAULT NULL COMMENT '备注信息，可记录额外的上下文信息',
    `created_at` DATETIME(3) DEFAULT NULL COMMENT '创建时间',
    `updated_at` DATETIME(3) DEFAULT NULL COMMENT '更新时间',
    `deleted_at` DATETIME(3) DEFAULT NULL COMMENT '软删除时间，不为空表示已删除',
    PRIMARY KEY (`id`),
    ...
    KEY `idx_app_type_created_at` (`app_type`, `created_at`),
    KEY `idx_error_type_created_at` (`error_type`, `created_at`),
